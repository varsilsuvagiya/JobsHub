...
import { otprequestr, verifyOtpr, setNewPasswordr } from "../api/common_api";
...
function Forgotpassword() {
  ...
  const OtpRequestFunction = async () => {
    ...
    const resp = await otprequestr(data?.email);
  ...
  const verifyOtpf = async () => {
    ...
    const resp = await verifyOtpr(data?.otp, data?.email);
